...
- **`spark.shuffle.streaming.maxBandwidthMBps`** is enforced by a per-`BackpressureProtocol`-instance token bucket whose refill rate is `maxBandwidthMBps / numConcurrentShuffles`. The default `-1` disables the rate limiter entirely; any positive integer N enables the cap at N MB/s. The value `0` is rejected at configuration parsing time because it would block all streaming shuffle traffic; the source-of-truth validator is `.checkValue(v => v == -1 || v > 0, ...)` in `core/src/main/scala/org/apache/spark/internal/config/package.scala`.
