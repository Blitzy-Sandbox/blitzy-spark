...
//     the verified identity — the canonical graph path, its size in bytes, and the project's own
//     recorded input path — is recorded in `diagnostics.graph_identity`. Which of import or open
//     happened is recorded in `diagnostics.load_mode`. Opening an existing project is still
//     reading.
